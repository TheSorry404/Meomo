...
export const useMemoStore = defineStore("memo", {
	...
	actions: {
		...
		async fetchMemos(
			params: PaginationParams & MemoFilter = { page: 1, pageSize: 20 }
		) {
			try {
				...
				const response = await apiService.post<
					ApiResponse<{ memos: Memo[]; total: number }>
				>(API_ENDPOINTS.MEMOS.LIST, params);
				...
				this.setError(
					error instanceof Error ? error.message : "Failed to fetch memos"
				);
			} finally {
		...
		async createMemo(
			memoData: Omit<Memo, "id" | "userId" | "createdAt" | "updatedAt">
		) {
			try {
				this.setLoading(true);
				this.setError(null);

				const response = await apiService.post<ApiResponse<Memo>>(
					API_ENDPOINTS.MEMOS.CREATE,
					memoData
				);
