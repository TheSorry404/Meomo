      ...
      <main class="dashboard-main">
        ...
        <div class="toolbar">
          ...
          <div class="toolbar-right">
            <button class="btn btn-outline" @click="showViewOptions = !showViewOptions">
              视图
            </button>
            <button class="btn btn-primary" @click="createNewMemo">
              + 新建备忘录
            </button>
          </div>
        </div>
        ...
        <div class="memos-section">
          ...
          <div v-else :class="`memos-${viewMode}`">
            <!-- 置顶备忘录 -->
            <div v-if="pinnedMemosList.length > 0" class="pinned-section">
            ...
            <!-- 普通备忘录 -->
            <div v-if="unpinnedMemosList.length > 0" class="regular-section">
              <h3 v-if="pinnedMemosList.length > 0" class="section-title">
                其他备忘录
              </h3>
              <div :class="`memos-${viewMode}`">
                <MemoCard
...
// 统计数据
const pinnedMemos = computed(() => memos.value.filter(m => m.isPinned).length)
...
const todayMemos = computed(() => {
  ...
  return memos.value.filter(m => new Date(m.createdAt).toDateString() === today).length
})
...
// 置顶和普通备忘录
const pinnedMemosList = computed(() => filteredMemos.value.filter(m => m.isPinned))
const unpinnedMemosList = computed(() => filteredMemos.value.filter(m => !m.isPinned))
